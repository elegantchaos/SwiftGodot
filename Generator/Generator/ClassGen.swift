...
func generateMethods(
    ...
    asSingleton: Bool
) -> [String: (String, JGodotClassMethod)] {
    p("/* Methods */")
...
func generateProperties(
    ...
    var okList: Set<String> = [
        "RefCounted", "Node", "Sprite2D", "Node2D", "CanvasItem", "Object", "String", "StringName", "AStar2D", "Material", "Camera3D", "Node3D", "ProjectSettings", "MeshInstance3D", "BoxMesh",
        "SceneTree", "Window", "Label", "Timer", "AudioStreamPlayer", "PackedScene", "PathFollow2D", "InputEvent", "ClassDB", "AnimatedSprite2D", "Input", "CollisionShape2D", "SpriteFrames",
        "RigidBody2D",
    ]
...
func getGenericSignalType(_ signal: JGodotSignal) -> String {
    ...
    return argTypes.isEmpty ? "SimpleSignal2" : "GenericSignal<\(argTypes.joined(separator: ", "))>"
}
...
func generateSignalType(_ p: Printer, _ cdef: JGodotExtensionAPIClass, _ signal: JGodotSignal, _ name: String) -> String {
    ...
    doc(
        p, cdef,
        "Use the ``\(name)/connect(flags:_:)`` method to connect to the signal on the container object, and ``\(name)/disconnect(_:)`` to drop the connection.\nYou can also await the ``\(name)/emitted`` property for waiting for a single emission of the signal."
    )
