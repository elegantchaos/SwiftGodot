...
// MARK: - Preparation
extension MemoryLayoutTests {

    fileprivate func getMetadata() throws -> ExtensionMetadata {
    ...
    fileprivate func prepareMemoryChecker<T>(for type: T.Type) throws -> MemoryLayoutChecker<T> {
...
extension MemoryLayoutTests {
    ...
    fileprivate final class MemoryLayoutChecker<T> {
    ...
    fileprivate struct ExtensionMetadata {
        let sizes: [JGodotSize]
        let offsets: [JGodotBuiltinClassMemberOffsetClass]
    }

    fileprivate enum MemoryTestError: Error {
