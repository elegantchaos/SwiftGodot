...
final class Vector2Tests: GodotTestCase {
    ...
    func testInterpolationMethods() {
        ...
        assertApproxEqual(
            vector1.angleTo(vector1.slerp(to: vector2, weight: 0.5)) * 2, vector1.angleTo(vector2), "Vector2 slerp with different length input should return a vector with an interpolated angle.")
        ...
        assertApproxEqual(
            vector1.cubicInterpolate(b: vector2, preA: Vector2(), postB: Vector2(x: 7, y: 7), weight: 1.0 / 3.0), Vector2(x: 1.851851940155029297, y: 2.962963104248046875),
            "Vector2 cubicInterpolate should work as expected.")
