...
final class Vector3Tests: GodotTestCase {
    ...
    func testInterpolationMethods() {
        ...
        assertApproxEqual(
            vector1.normalized().slerp(to: vector2.normalized(), weight: 0.5), Vector3(x: 0.363866806030273438, y: 0.555698215961456299, z: 0.747529566287994385),
            "Vector3 slerp should work as expected.")
        assertApproxEqual(
            vector1.normalized().slerp(to: vector2.normalized(), weight: 1.0 / 3.0), Vector3(x: 0.332119762897491455, y: 0.549413740634918213, z: 0.766707837581634521),
            "Vector3 slerp should work as expected.")
        assertApproxEqual(
            Vector3(x: 5, y: 0, z: 0).slerp(to: Vector3(x: 0, y: 3, z: 4), weight: 0.5), Vector3(x: 3.535533905029296875, y: 2.121320486068725586, z: 2.828427314758300781),
        ...
        assertApproxEqual(
            Vector3(x: 4, y: 6, z: 2).slerp(to: Vector3(x: 8, y: 10, z: 3), weight: 0.5), Vector3(x: 5.90194219811429941053, y: 8.06758688849378394534, z: 2.558307894718317120038),
            "Vector3 slerp should work as expected.")
        ...
        assertApproxEqual(
            vector1.angleTo(vector1.slerp(to: vector2, weight: 0.5)) * 2, vector1.angleTo(vector2), "Vector3 slerp with different length input should return a vector with an interpolated angle.")
        XCTAssertEqual(
            vector1.cubicInterpolate(b: vector2, preA: Vector3(), postB: Vector3(x: 7, y: 7, z: 7), weight: 0.5), Vector3(x: 2.375, y: 3.5, z: 4.625),
            "Vector3 cubicInterpolate should work as expected.")
        assertApproxEqual(
            vector1.cubicInterpolate(b: vector2, preA: Vector3(), postB: Vector3(x: 7, y: 7, z: 7), weight: 1.0 / 3.0),
            Vector3(x: 1.851851940155029297, y: 2.962963104248046875, z: 4.074074268341064453), "Vector3 cubicInterpolate should work as expected.")
    ...
    func testOperators() {
        ...
        XCTAssertEqual(
            Variant(Vector3(x: Float.e, y: Float.sqrt2, z: Float.sqrt3)).description, "(2.718282, 1.414214, 1.732051)",
            "Vector3 cast to String should print the correct amount of digits for realT = float.")
    }
    ...
    func testOtherMethods() {
        ...
        assertApproxEqual(
            vector.rotated(axis: Vector3(x: 1, y: 0, z: 0), angle: Double.tau / 3), Vector3(x: 1.2, y: -6.54974226119285642, z: 0.1444863728670914), "Vector3 rotated should work as expected.")
        assertApproxEqual(
            vector.rotated(axis: Vector3(x: 0, y: 0, z: 1), angle: Double.tau / 2), vector.rotated(axis: Vector3(x: 0, y: 0, z: 1), angle: Double.tau / -2), "Vector3 rotated should work as expected.")
    ...
    func testPlaneMethods() {
        ...
        assertApproxEqual(
            vector.bounce(n: vectorNormal), Vector3(x: -6.0369629829775736287, y: 1.25571467171034855444, z: 2.517589840583626047), "Vector3 bounce with normal should return expected value.")
        ...
        assertApproxEqual(
            vector.reflect(n: vectorNormal), Vector3(x: 6.0369629829775736287, y: -1.25571467171034855444, z: -2.517589840583626047), "Vector3 reflect with normal should return expected value.")
        ...
        assertApproxEqual(
            vector.project(b: vectorNormal), Vector3(x: 3.61848149148878681437, y: 1.0721426641448257227776, z: 1.54120507970818697649), "Vector3 projected on a normal should return expected value.")
        ...
        assertApproxEqual(
            vector.slide(n: vectorNormal), Vector3(x: -2.41848149148878681437, y: 2.32785733585517427722237, z: 4.0587949202918130235), "Vector3 slide with normal should return expected value.")
