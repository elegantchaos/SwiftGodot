...
final class Transform2DTests: GodotTestCase {
    ...
    func testInterpolation() {
        let rotateScaleSkewPos: Transform2D = Transform2D(
            rotation: Float(170.0).degreesToRadians, scale: Vector2(x: 3.6, y: 8.0), skew: Float(20.0).degreesToRadians, position: Vector2(x: 2.4, y: 6.8))
        let rotateScaleSkewPosHalfway: Transform2D = Transform2D(
            rotation: Float(85.0).degreesToRadians, scale: Vector2(x: 2.3, y: 4.5), skew: Float(10.0).degreesToRadians, position: Vector2(x: 1.2, y: 3.4))
    ...
    func testIsConformalChecks() {
        ...
        XCTAssertTrue(
            Transform2D(xAxis: Vector2(x: 1.2, y: 3.4), yAxis: Vector2(x: 3.4, y: -1.2), origin: Vector2()).isConformal(), "Transform2D with a flip, rotation, and uniform scale should be conformal.")
        ...
        XCTAssertFalse(
            Transform2D(xAxis: Vector2(x: Float.sqrt12, y: Float.sqrt12), yAxis: Vector2(x: 0, y: 1), origin: Vector2()).isConformal(),
